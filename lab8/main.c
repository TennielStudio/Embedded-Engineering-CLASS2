...
int main(void) {
    ...
    while(1) {
        ...
        SysCtlDelay(SysCtlClockGet()/5); //light delay
        ...
        fill_frame_buffer(255, 255, 0, NUM_LEDS); //Red Green
        send_data(frame_buffer, NUM_LEDS);

        SysCtlDelay(SysCtlClockGet()/5);//light delay

        fill_frame_buffer(255, 0, 255, NUM_LEDS); //Red Blue
        send_data(frame_buffer, NUM_LEDS);

        SysCtlDelay(SysCtlClockGet()/5);//light delay

        fill_frame_buffer(0, 255, 255, NUM_LEDS); //Green Blue
        send_data(frame_buffer, NUM_LEDS);

        SysCtlDelay(SysCtlClockGet()/5);//light delay

        fill_frame_buffer(255, 255, 255, NUM_LEDS); //Red Green Blue
        send_data(frame_buffer, NUM_LEDS);
